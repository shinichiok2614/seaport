  ...
  return (
    <div>
      <h2 id="department-heading" data-cy="DepartmentHeading">
        <Translate contentKey="seaportApp.department.home.title">Departments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="seaportApp.department.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/department/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="seaportApp.department.home.createLabel">Create new Department</Translate>
          </Link>
        </div>
      </h2>
        ...
        {departmentList && departmentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="seaportApp.department.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
              ...
              {departmentList.map((department, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {department.image ? (
                      <div>
                        {department.imageContentType ? (
                          <a onClick={openFile(department.imageContentType, department.image)}>
                            <img src={`data:${department.imageContentType};base64,${department.image}`} style={{ maxHeight: '30px' }} />
                            &nbsp;
                          </a>
                        ) : null}
                        <span>
                          {department.imageContentType}, {byteSize(department.image)}
                        </span>
                      </div>
                    ) : null}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="seaportApp.department.home.notFound">No Departments found</Translate>
            </div>
          )
        )}
      </div>
