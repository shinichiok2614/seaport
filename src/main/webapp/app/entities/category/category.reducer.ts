...
export const getEntityWithPost = createAsyncThunk(
  'category/fetch_entity_with_post',
  async () => {
    const requestUrl = `${apiUrl}/categories-with-posts`;
    return axios.get<ICategory[]>(requestUrl);
  },
  { serializeError: serializeAxiosError },
);
...
export const updateEntity = createAsyncThunk(
  ...
  async (entity: ICategory, thunkAPI) => {
    const result = await axios.put<ICategory>(
      `${apiUrl}/${entity.id}`,
      cleanEntity(entity),
    );
    thunkAPI.dispatch(getEntities({}));
...
export const partialUpdateEntity = createAsyncThunk(
  ...
  async (entity: ICategory, thunkAPI) => {
    const result = await axios.patch<ICategory>(
      `${apiUrl}/${entity.id}`,
      cleanEntity(entity),
    );
    thunkAPI.dispatch(getEntities({}));
...
export const CategorySlice = createEntitySlice({
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(getEntityWithPost.fulfilled, (state, action) => {
        state.loading = false;
      ...
      .addMatcher(
        isPending(getEntities, getEntity, getEntityWithPost),
        state => {
